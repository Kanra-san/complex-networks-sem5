...
def remove_edges_for_infectious_node(G, node, edge_removal_prob=0.5):
    ...
    for day in range(total_days):
        ...
        for node in G.nodes:
            ...
            if 'day_infected' in G.nodes[node]:
                ...
                if status == 'E' and days_infected >= incubation_period:
...
def plot_network(G, day, directory):
...
for day in range(simulation_days + 1):
    ...
    r_progress.append(count_nodes_status(G, 'R'))
        ...
        #plot_network(G, day, 'ws')

plot_progress(s_progress, e_progress, i_progress, r_progress, "progress_ws")
...
for day in range(simulation_days + 1):
    ...
    s_progress.append(count_nodes_status(G2, 'S'))
    e_progress.append(count_nodes_status(G2, 'E'))
    i_progress.append(count_nodes_status(G2, 'I'))
    r_progress.append(count_nodes_status(G2, 'R'))
    #if day in range(simulation_days + 1):  # [0, simulation_days // 2, simulation_days]:
        #plot_network(G2, day, 'er')

plot_progress(s_progress, e_progress, i_progress, r_progress, "progress_er")
